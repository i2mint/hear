...
def wf_of_block_bytes(block_bytes, sr):
    return sf.read(BytesIO(block_bytes), dtype='int16', channels=1,
                   samplerate=sr, format='RAW', subtype='PCM_16')[0]
...
class SessionBlockStore(LocalPathStore):
    ...
    def get_session_and_block(self, k):
        ...
        if m:
            d = m.groupdict()
            return int_if_not_none(d.get('session', None)), int_if_not_none(d.get('block', None))
        else:
            return None, None
...
class TxyzStore(SessionBlockStore):
    ...
    def df_from_json_lines(data):
        import pandas as pd
        return pd.DataFrame(list(map(json.loads, filter(None, data.split('\n'))))).set_index('timestamp')
...
class BlocksSearchResult:
    def __init__(self, _block_store, bt=-inf, tt=inf, _inclusive=(True, False), _sort_key=None):
        """
        ...
        self.time_units_per_sec = self._block_store.time_units_per_sec  # sample rate of the query time unit
    ...
    def intersecting_blocks(self):
        ...
        return list(self._key_of_block.irange(
            minimum=min_block, maximum=self.tt, inclusive=self._inclusive))

    def __iter__(self):  # TODO: Change to be the bts of the values returned by items, not block bts.
        return self.items()
    ...
    def blocks_intersecting_bt_tt(self):
        ...
        if bottom_idx > 0:
            bottom_idx -= 1
        min_block = self._key_of_block[bottom_idx]
        return self._key_of_block.irange(minimum=min_block, maximum=self.tt, inclusive=self._inclusive)
    ...
    def items(self):
        ...
        try:
            ...
            while current_block_ts < self.tt:
                ...
                remove_right_ts_unit = max(0, current_block_ts + wf_len_ts_unit - self.tt)
                ...
                remove_right_sample_unit = int(remove_right_ts_unit * samples_per_ts_unit)
...
class BlockWfStore(DictKeyMap, WfStore):
    def __init__(self, channel_data_dir, sr=None, time_units_per_sec=DFLT_TIME_UNITS_PER_SEC, _sort_key=None):
    ...
    def block_search(self, bt=-inf, tt=inf):
        return BlocksSearchResult(_block_store=self, bt=bt, tt=tt, _inclusive=(True, False), _sort_key=self._sort_key)
...
class ScoopAnnotationsStore(SessionStore):
    ...
    def __getitem__(self, k):
        ...
        csv_timestamp = int(csv_timestamp * self.time_units_per_sec / self.csv_timestamp_time_units_per_sec)
